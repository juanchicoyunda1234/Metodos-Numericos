...
function MethodCard({
  ...
  return (
    <article className="flex flex-col gap-4 rounded-box border border-border bg-panel p-5">
    ...
    <div className="flex flex-col gap-5">
      {!hasResults && (
        <EmptyState
          ...
          hint="Usa la misma f(x) y los mismos parámetros, luego pulsa Ejecutar. Verás iteraciones, evaluaciones de derivada y el error en escala logarítmica."
        />
      )}
      ...
      {classic && constant && (
        <p className="max-w-[75ch] text-sm leading-relaxed text-text">{comparisonConclusion(classic, constant)}</p>
      )}

      <div className="flex flex-col gap-2">
        <h2 className="text-sm font-medium text-text">Error vs iteración</h2>
        <ConvergenceChart option={errorOption} height={280} colorScheme={colorScheme} />
      </div>

      <div className="flex flex-col gap-2">
        <h2 className="text-sm font-medium text-text">Trayectorias superpuestas</h2>
